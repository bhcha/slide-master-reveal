...
1. 스캐폴드 스크립트 실행:
   ```bash
   node <plugin-dir>/scripts/create-from-export.js \
...
2. 생성된 파일 확인:
   - `index.html` — reveal.js HTML (레이아웃 래퍼 + `<!-- AI: ... -->` placeholder)
   - `styles.css` — Slide Master CSS + reveal.js override

3. 결과 보고: 몇 장의 슬라이드가 생성되었는지, 각 슬라이드의 레이아웃과 content/prompts 요약

## 참고

- `<plugin-dir>`은 이 파일이 위치한 플러그인 디렉토리의 상위 경로입니다.
- 스크립트 경로 예시: `~/workspace/slide-master-reveal/scripts/create-from-export.js`
- content 필드 → HTML에 직접 배치됨
- prompts 필드 → `<!-- AI: 프롬프트 -->` 주석으로 마킹됨
