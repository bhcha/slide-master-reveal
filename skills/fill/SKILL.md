...
> **CRITICAL — 두 가지 절대 규칙:**
>
> 1. **구조를 건드리지 마세요.** scaffold가 만든 레이아웃 래퍼(`layout-*`), `.slide`, `.slide-header`, `.slide-body`, `.slide-footer`를 수정/교체/재구성하지 마세요. 콘텐츠는 **`.slide-body` 안에만** 넣으세요.
> 2. **1280×720px을 넘기지 마세요.** 넘치면 잘립니다. 내용이 많으면 슬라이드를 나누세요.
...
scaffold가 만든 HTML 구조는 Studio 디자인 토큰과 연결되어 있습니다. 이것을 건드리면 토큰 시스템이 깨집니다.

**절대 하지 마세요:**
- `layout-*` 클래스 변경 (예: `layout-header-body-footer` → `layout-blank-canvas`)
- `.slide`의 `background`, `color`, `padding`, `display`, `grid-template-rows` 인라인 오버라이드
- `.slide-header`, `.slide-footer`, `.slide-body`의 구조 변경 (삭제, 이동, 속성 오버라이드)
- 색상을 하드코딩 (예: `#1A2332`, `rgba(255,255,255,0.9)`) — 반드시 `var(--slide-color-*)` 토큰 사용
- 폰트를 인라인으로 지정 — export CSS가 관리

**해도 되는 것:**
- `.slide-body` 안에 HTML 요소 추가
- `.slide-body` 내부에서 `display: grid; grid-template-columns: ...` 사용 (콘텐츠 배치용)
- export CSS의 클래스 사용 (`.card`, `.badge`, `.slide-grid-2`, `.text-lg` 등)
- `var(--slide-color-*)` 토큰 참조
