...
`slides/slide-N-notes.html`에 텍스트를 작성합니다. assemble 후 `<aside class="notes">`에 삽입됩니다.
발표자 뷰(S키)에서만 표시됩니다.

### 나레이션 바

슬라이드 화면에 직접 보이는 토스트 형태의 나레이션입니다. body 파일에 추가:
```html
<div class="slide-narration-wrap">
  <div class="slide-narration-bar">나레이션 텍스트</div>
</div>
```
